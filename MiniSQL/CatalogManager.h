...
namespace Catalog
{
	...
	public:
		...
		// 索引名若为 noIndex = "#NULL#" 会被自动替换成 "TableName#ColumnName"
		int CreateIndex(std::string tableName, std::string columnName, std::string indexName = noIndex);
		// 删除索引,返回类型 Table_name#Column_name:success noIndex:找不到对应的索引
